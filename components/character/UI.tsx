    ...
    <main className="pointer-events-none fixed z-10 inset-0 select-none">
      ...
      <div className="mx-auto h-full max-w-screen-xl w-full flex flex-col justify-between">
        <div className="flex justify-between items-center p-10">
          <div className="text-white text-xl font-bold pointer-events-auto">
            Character Creator
          </div>
...
const PosesBox = () => {
  ...
  return (
    <div className="pointer-events-auto md:rounded-t-lg bg-gradient-to-br from-black/30 to-indigo-900/20 backdrop-blur-sm drop-shadow-md flex p-6 gap-3 overflow-x-auto">
...
const AssetsBox = () => {
  const { categories, currentCategory, setCurrentCategory, changeAsset, customization, lockedGroups } =
  ...
  return (
    <div className="md:rounded-t-lg bg-gradient-to-br from-black/30 to-indigo-900/20 backdrop-blur-sm drop-shadow-md flex flex-col py-6 gap-3 overflow-hidden">
      <div className="flex items-center gap-8 pointer-events-auto overflow-x-auto px-6 pb-2">
      ...
      {currentCategory && lockedGroups[currentCategory.name] && (
        <p className="text-red-400 px-6">
          ...
          {lockedGroups[currentCategory.name]
            .map((asset: any) => `${asset.name} (${asset.categoryName})`)
            .join(", ")}
        </p>
      )}
        ...
        {currentCategory?.assets.map((asset) => (
          ...
          >
            {asset.thumbnailUrl ? (
              ...
              <div className="w-full h-full bg-gray-700 flex items-center justify-center text-white text-xs p-1">
                {asset.name}
              </div>
            )}
          </button>
...
const ColorPicker = () => {
  ...
  return (
    <div className="pointer-events-auto relative flex gap-2 max-w-full overflow-x-auto backdrop-blur-sm py-2 drop-shadow-md px-2 md:px-0">
